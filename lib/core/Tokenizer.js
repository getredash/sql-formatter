...
var Tokenizer = function() {
   ...
   *  @param {String[]} cfg.reservedToplevelWords Words that are set to new line
   * separately
   ...
   *  @param {String[]} cfg.indexedPlaceholderTypes Prefixes for indexed
   * placeholders, like ?
   *  @param {String[]} cfg.namedPlaceholderTypes Prefixes for named
   * placeholders, like @ and :
   *  @param {String[]} cfg.lineCommentTypes Line comments to enable, like # and
   * --
   *  @param {String[]} cfg.specialWordChars Special chars that can be found
   * inside of words, like @ and #
   */
  function Tokenizer(cfg) {
    ...
    this.INDEXED_PLACEHOLDER_REGEX =
        this.createPlaceholderRegex(cfg.indexedPlaceholderTypes, "[0-9]*");
    ...
    this.STRING_NAMED_PLACEHOLDER_REGEX = this.createPlaceholderRegex(
        cfg.namedPlaceholderTypes, this.createStringPattern(cfg.stringTypes));
  }
  ...
  Tokenizer.prototype.createLineCommentRegex = function createLineCommentRegex(
    ...
    return new RegExp(
        "^((?:" +
        lineCommentTypes
            .map(function(c) { return (0, _escapeRegExp2["default"])(c); })
            .join("|") +
        ").*?(?:\n|$))");
  };
  ...
  Tokenizer.prototype.createParenRegex = function createParenRegex(parens) {
    ...
    return new RegExp(
        "^(" +
            parens.map(function(p) { return _this.escapeParen(p); }).join("|") +
            ")",
        "i");
  };
  ...
  Tokenizer.prototype.getNextToken = function getNextToken(input,
                                                           previousToken) {
    return this.getWhitespaceToken(input) || this.getCommentToken(input) ||
           this.getStringToken(input) || this.getOpenParenToken(input) ||
           this.getCloseParenToken(input) || this.getPlaceholderToken(input) ||
  ...
  Tokenizer.prototype.getReservedWordToken = function getReservedWordToken(
    ...
    // this makes it so in "mytable.from", "from" is not considered a reserved
    // word
